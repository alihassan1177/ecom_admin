...
import { getDataFromApi, BASE_APP_URL, deleteData } from "../../utils/Api";
...
export default function ProductPage() {
  ...
  async function deleteRow(id) {
    await deleteData(`product/delete/${id}`, user.token);
    setProducts(await getDataFromApi("/products", user.token));
  }
  ...
  return (
    <MainLayout>
      ...
      <div className="m-4 p-4 border rounded">
        ...
        <table
          className={`bg-white ${
            products.length > 1 || products.length < 1 ? "min-h-[150px]" : ""
          }  rounded border overflow-hidden relative p-2 w-full`}
        >
          ...
          <tbody className="relative transition-all">
            {loading ? (
              ...
              <tr className="absolute left-1/2 top-16 -translate-x-1/2">
